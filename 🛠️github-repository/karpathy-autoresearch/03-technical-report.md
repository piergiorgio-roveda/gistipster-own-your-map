---
repo: karpathy/autoresearch
url: https://github.com/karpathy/autoresearch
date_analysis: 2026-03-28
version: 1
analyst: pipeline/report-builder
status: draft
---

# karpathy/autoresearch — Valutazione Tecnica

> **Repository**: [karpathy/autoresearch](https://github.com/karpathy/autoresearch) | **Data
> analisi**: 2026-03-28 | **Versione**: 1

---

## Indice

| #   | Sezione               | Disponibilità    |
| --- | --------------------- | ---------------- |
| 1   | Quick Summary         | obbligatoria     |
| 2   | Informazioni Generali | obbligatoria     |
| 3   | Attività di Sviluppo  | dati disponibili |
| 4   | Contribuenti          | dati disponibili |
| 8   | Issues & PR           | dati disponibili |
| A1  | Analisi LLM           | allegato         |

---

## Quick Summary

| Indicatore | Status    | Note                                                    |
| ---------- | --------- | ------------------------------------------------------- |
| Salute     | 🔴 3.3/10 | score composito (attività + BF + issue + release + sec) |
| Attività   | 🟢 Attivo | ultimo commit: 2026-03-26                               |
| Bus Factor | 🔴 1      | contributor dominanti                                   |
| Sicurezza  | ⚪ N/D    | OpenSSF Scorecard                                       |
| CI/CD      | ⚪ N/D    | da topics repo                                          |
| Community  | 🟢 58.523 | ★ stars                                                 |

---

## 1. Informazioni Generali

| Campo       | Valore                                                                           |
| ----------- | -------------------------------------------------------------------------------- |
| Repository  | [karpathy/autoresearch](https://github.com/karpathy/autoresearch)                |
| Categoria   | Backend                                                                          |
| Licenza     | —                                                                                |
| Linguaggio  | Python                                                                           |
| Stars       | 58.523                                                                           |
| Forks       | 8105                                                                             |
| Watchers    | 481                                                                              |
| Open Issues | 156                                                                              |
| Creato      | 2026-03-06 (1 mese fa)                                                           |
| Archivio    | No                                                                               |
| Fork        | No                                                                               |
| DeepWiki    | [deepwiki.com/karpathy/autoresearch](https://deepwiki.com/karpathy/autoresearch) |

---

## 3. Attività di Sviluppo

| Metrica            | Valore        |
| ------------------ | ------------- |
| Ultimo commit      | 2026-03-26    |
| Commit (30 giorni) | 10            |
| Commit (90 giorni) | 10            |
| Trend commit       | ↑ In crescita |

---

## 4. Contribuenti

Totale contributors: **8** (8 umani, 0 bot)

**Bus Factor**: 🔴 1 — contributor con >10% dei commit totali

| #   | Contributor      | Contribuzioni | %     | Tipo |
| --- | ---------------- | ------------- | ----- | ---- |
| 1   | @karpathy        | 28            | 80.0% | User |
| 2   | @dipeshbabu      | 1             | 2.9%  | User |
| 3   | @hughdbrown      | 1             | 2.9%  | User |
| 4   | @marcinbogdanski | 1             | 2.9%  | User |
| 5   | @dumko2001       | 1             | 2.9%  | User |
| 6   | @haosenwang1018  | 1             | 2.9%  | User |
| 7   | @indianspeedster | 1             | 2.9%  | User |
| 8   | @kaizen-38       | 1             | 2.9%  | User |

---

## 8. Issues & PR

| Metrica             | Valore |
| ------------------- | ------ |
| Issue aperte        | 156    |
| Issue chiuse (90gg) | 0      |

---

## A1 — Analisi LLM

> _Analisi qualitativa dettagliata disponibile in `02-llm-analysis.md`._

---
